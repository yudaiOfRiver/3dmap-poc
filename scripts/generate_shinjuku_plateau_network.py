"""
新宿駅PLATEAUデータの歩行ネットワークグラフを生成するスクリプト

B4〜2Fのフロアデータからfloor・doorサーフェスのcentroidを
ノードとして抽出し、KNN + 距離閾値でリンクを生成する。
フロア間はdoor付近の垂直接続も追加する。

出力: public/data/shinjuku_plateau/network.json
"""
...
# 設定
# ──────────────────────────────────────────────
MAX_LINK_DISTANCE = 25.0  # 同一フロア内リンクの最大距離 (m) ※ノード密度が高いため渋谷版より絞る
K_NEIGHBORS = 6           # 各ノードから接続する最大近傍数
VERTICAL_ALIGN_XZ = 5.0   # フロア間接続のXZ距離閾値 (m)

BASE_DIR = Path(__file__).resolve().parent.parent
DATA_DIR = BASE_DIR / "public" / "data" / "shinjuku_plateau"
OUTPUT_PATH = DATA_DIR / "network.json"

# index.json から読み込んでもよいが、明示的に定義
FLOORS = {
    "B4": {"file": "B4.json", "y": -20.0},
    "B3": {"file": "B3.json", "y": -15.0},
    "B2": {"file": "B2.json", "y": -10.0},
    "B1": {"file": "B1.json", "y": -5.0},
    "0":  {"file": "0.json",  "y": 0.0},
    "1":  {"file": "1.json",  "y": 5.0},
    "2":  {"file": "2.json",  "y": 10.0},
}
...
def generate_intra_floor_links(nodes):
    ...
    for floor_name, fnodes in by_floor.items():
        print(f"    {floor_name}: {len(fnodes)} nodes ...", end="", flush=True)
        ...
        print(f" done ({len(links)} total links so far)")
...
def generate_inter_floor_links(nodes):
    ...
    for f_upper, f_lower in FLOOR_PAIRS:
        ...
        print(f"    {f_upper} <-> {f_lower}: {pair_count} links")
